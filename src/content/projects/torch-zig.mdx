...
id: torch-zig
title: "torch-zig"
summary: "Zig bindings for the C++ api of PyTorch"
imageUrl: "/images/torch-zig.png"
tags: ["C++", "Zig"]
featured: false
date: 2024-07-25
---
...
> I AM NOT RESPONSIBLE FOR ANY DAMAGE CAUSED BY THIS CODE. THAT INCLUDES EMOTIONAL ONES TOO.
            ...
            <div className="project-repo-card">
              <RepoCard username="theunnecessarythings" repository="torch-zig" />
            </div>
...
<div className="project-code-wrap">
    <CodeWindow  title="Usage" language="bash" code={`zig build lib # Builds the C++ bindings to a static library, Only needs to be run once
...
<CodeWindow title="Fetch the library" language="bash" code={`# Fetch the library
...
<CodeWindow title="Build" language="zig" code={`const comp: []const u8 = "clang++";
...
<CodeWindow title="Usage" language="zig" code={`const torch = @import("torch");
...
<CodeWindow title="Output" language="bash" 
code={`0.3737 -2.6723 -0.0300
...
<CodeWindow title="Freeing a tensor" language="bash"
code={`const a = Tensor.randn(&.{3, 3}, torch.FLOAT_CPU);
a.free();`}/>

Except its not!!!
<CodeWindow title="Freeing a tensor" language="zig"
code={`const a = Tensor.randn(&.{3, 3}, torch.FLOAT_CPU);
...
<CodeWindow title="TensorPool" language="zig"
code={`const size = torch.memory_pool.getPoolSize("default"); // Returns memory usage of the default pool
...
<CodeWindow title="NoGradGuard" language="zig" code={`{
...
## Available Neural Network Layers
  ...
  <div className="project-checklist-wrap">
    <ul className="space-y-3">
      {[
  { text: 'Identity → <code>torch.linear.Identity</code>', checked: true },
  { text: 'Linear → <code>torch.linear.Linear</code>', checked: true },
  { text: 'Flatten → <code>torch.linear.Flatten</code>', checked: true },
  { text: 'Unflatten → <code>torch.linear.Unflatten</code>', checked: true },
  { text: 'Bilinear → <code>torch.linear.Bilinear</code>', checked: true },
  { text: 'Conv1D → <code>torch.conv.Conv1D</code>', checked: true },
  { text: 'Conv2D → <code>torch.conv.Conv2D</code>', checked: true },
  { text: 'Conv3D → <code>torch.conv.Conv3D</code>', checked: true },
  { text: 'ConvTranspose1D → <code>torch.conv.ConvTranspose1D</code>', checked: false },
  { text: 'ConvTranspose2D → <code>torch.conv.ConvTranspose2D</code>', checked: false },
  { text: 'ConvTranspose3D → <code>torch.conv.ConvTranspose3D</code>', checked: false },
  { text: 'Embedding → <code>torch.embedding.Embedding</code>', checked: true },
  { text: 'Sequential → <code>torch.module.Sequential</code>', checked: true },
  { text: 'BatchNormND → <code>torch.norm.BatchNorm(D)</code>', checked: true },
  { text: 'InstanceNormND → <code>torch.norm.InstanceNorm(D)</code>', checked: true },
  { text: 'LayerNorm → <code>torch.norm.LayerNorm</code>', checked: true },
  { text: 'GroupNorm → <code>torch.norm.GroupNorm</code>', checked: true },
  { text: 'Dropout → <code>torch.functional.Dropout</code>', checked: true },
].map((item, idx) => (
...
<CodeWindow title="Functional API" language="zig" 
code={
...
<CodeWindow title="Vision Models" language="zig" 
code={`
...
### Implemented Vision Models 
  <div className="project-checklist-wrap">
    <ul className="space-y-3">
      { [
  { text: 'Alexnet', checked: true },
  { text: 'ConvNext', checked: true },
  { text: 'DenseNet', checked: true },
  { text: 'EfficientNet - V2S, V2M, V2L → Testing Fails', checked: true },
  { text: 'Inception', checked: true },
  { text: 'MnasNet', checked: true },
  { text: 'MobileNetV2', checked: true },
  { text: 'MobileNetV3', checked: true },
  { text: 'RegNet', checked: false },
  { text: 'ResNet', checked: true },
  { text: 'ShuffleNetV2', checked: true },
  { text: 'SqueezeNet', checked: true },
  { text: 'SwinTransformer → Testing Fails', checked: true },
  { text: 'VGG', checked: true },
  { text: 'VisionTransformer', checked: true },
].map((item, idx) => (
        <ChecklistItem key={idx} text={item.text} checked={item.checked} />
      ))}
    </ul>
...
<CodeWindow title="Example MNIST Training" language="zig"
code={`
